<template>
    ...
    <!--p v-if="attendeeInput?.orgUrn ?? urn?.length > 0" class="absolute right-8 top-4 text-gray-600 text-xs">This school is verified and linked to <a :href="`https://get-information-schools.service.gov.uk/Establishments/Establishment/Details/${urn}`" class="text-gray-600 underline" target="_blank">government establishment data</a></p-->

    <status-approved :approved="parseInt(approved)" :urn="urn" :disabled="(urn?.length > 0 ? false : true)" />
    ...
    >
        ...
        <div class="text-right">
            <span role="button" class="inline-block bg-red-300 text-red-800 font-bold py-2 px-3 mr-2 text-sm rounded-lg cursor-pointer" @click="hideForm">Cancel</span>
            <button
                v-if="!hideAnother"
